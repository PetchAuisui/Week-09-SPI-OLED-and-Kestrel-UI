...
#### หมุนประมาณ 45 องศา (ระดับ 25%)
อ่านค่าได้ RAW 1104 คำนวณได้ 25.1% แถบเกจบนเว็บและจอ OLED ขึ้นตรงกันประมาณหนึ่งในสี่

![ภาพผลการทดลองที่ระดับ 25 เปอร์เซ็นต์](Images/Lab9-3-dashboard-25pct.jpg)

#### กึ่งกลาง 90 องศา (ระดับ 50%)
อ่านค่าได้ RAW 2055 คำนวณได้ 50.1% แถบเกจบนเว็บและจอ OLED ขึ้นครึ่งหลอดตรงกัน

![ภาพผลการทดลองที่ระดับ 50 เปอร์เซ็นต์](Images/Lab9-3-dashboard-50pct.jpg)

#### มุมประมาณ 135 องศา (ระดับ 75%)
อ่านค่าได้ RAW 3008 คำนวณได้ 75.2% แถบเกจบนเว็บและจอ OLED ขึ้นตรงกันสามในสี่

![ภาพผลการทดลองที่ระดับ 75 เปอร์เซ็นต์](Images/Lab9-3-dashboard-75pct.jpg)
...
| --- | --- | --- | --- | --- | --- |
| หมุนซ้ายสุด (0 องศา) | 0 | 0.0 % | 0.0 % | ตรงกันสมบูรณ์ | CLOUD Siwarpat Auisu |
| หมุนประมาณ 45 องศา | 1104 | 25.1 % | 25.1 % | ตรงกันสมบูรณ์ | CLOUD Siwarpat Auisu |
| กึ่งกลาง (90 องศา) | 2055 | 50.1 % | 50.1 % | ตรงกันสมบูรณ์ | CLOUD Siwarpat Auisu |
| หมุนประมาณ 135 องศา | 3008 | 75.2 % | 75.2 % | ตรงกันสมบูรณ์ | CLOUD Siwarpat Auisu |
| หมุนขวาสุด (180 องศา) | 4095 | 100.0 % | 100.0 % | ตรงกันสมบูรณ์ | CLOUD Siwarpat Auisu |
...
จากการทดสอบยิงคำสั่งเปลี่ยนข้อความหน้าจอผ่าน HTTP POST `/api/oled/message` ด้วยคำสั่ง `curl` บน Terminal พร้อมคำนวณค่าเป็น `TotalMilliseconds`:

```bash
curl -s -o /dev/null -w "%{time_total}\n" \
  -X POST http://localhost:5000/api/oled/message \
  -H "Content-Type: application/json" \
  -d '{"message":"PING TEST"}' | awk '{printf "TotalMilliseconds: %.3f ms\n", $1 * 1000}'
```

ผลการทดสอบจับเวลาบนเครื่องจริงได้ค่า:
```text
TotalMilliseconds: 1.644 ms
```

![ภาพผลการตรวจวัดเวลา TotalMilliseconds ด้วย curl และ awk บน Terminal](Images/Lab9-3-latency-ping.png)

**บันทึกค่า TotalMilliseconds ที่ได้**

- เวลาที่ Kestrel ใช้ในการรับคำขอและอัปเดตสถานะ **`1.64 ms`** (ผลการวัดจริงได้ `1.644 ms`)  
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial ไปจนถึงจังหวะที่จอ OLED วาดข้อความใหม่สำเร็จ **`3.82 ms`** 
> คำนวณจากเวลาส่งแพ็กเก็ตผ่านสาย Serial UART 115200 bps ประมาณ 3.0 ms + เวลาที่ ESP32 ส่งข้อมูล Framebuffer 1024 ไบต์ขึ้นจอ OLED ผ่านบัส SPI 10 MHz อีกประมาณ 0.82 ms
